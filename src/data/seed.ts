import type { AppState, Week, StoryCard, DSAProblem, SystemDesignTopic, Application, MockInterview, Note } from '../types';
...
const weeks: Week[] = [
  {
    ...
    tasks: [
      ...
      { id: uid(), weekId: 'w1', title: 'Tag stories by topic (performance, reliability, architecture, etc.)', status: 'todo', notes: '' },
      { id: uid(), weekId: 'w1', title: 'Solve 20 easy LeetCode problems (Arrays, Strings, Hashmaps)', status: 'todo', notes: '' },
  ...
  },
  {
    ...
    tasks: [
      ...
      { id: uid(), weekId: 'w2', title: 'Review pattern notes and add to DSA notes section', status: 'todo', notes: '' },
    ]
  },
  {
    id: 'w3', number: 3,
    ...
    tasks: [
      { id: uid(), weekId: 'w3', title: 'Binary Trees: traversals, LCA, diameter (10 problems)', status: 'todo', notes: '' },
      { id: uid(), weekId: 'w3', title: 'BST: insert, delete, validate, kth smallest (6 problems)', status: 'todo', notes: '' },
      { id: uid(), weekId: 'w3', title: 'Graph BFS/DFS: connected components, islands (8 problems)', status: 'todo', notes: '' },
      { id: uid(), weekId: 'w3', title: 'Shortest paths: Dijkstra, Bellman-Ford (4 problems)', status: 'todo', notes: '' },
      { id: uid(), weekId: 'w3', title: 'Heap/Priority Queue: top-K, merge K lists (6 problems)', status: 'todo', notes: '' },
      { id: uid(), weekId: 'w3', title: 'DP Introduction: Fibonacci, climb stairs, coin change (6 problems)', status: 'todo', notes: '' },
    ]
  },
  {
    id: 'w4', number: 4,
    ...
    tasks: [
      ...
      { id: uid(), weekId: 'w4', title: 'Study: Caching strategies (write-through, write-back, LRU)', status: 'todo', notes: '' },
  ...
  },
  {
    ...
    tasks: [
      { id: uid(), weekId: 'w5', title: 'Study: Message queues (Kafka, RabbitMQ) and async processing', status: 'todo', notes: '' },
      { id: uid(), weekId: 'w5', title: 'Study: Event-driven architecture and CQRS', status: 'todo', notes: '' },
      { id: uid(), weekId: 'w5', title: 'Study: Data consistency patterns in distributed systems', status: 'todo', notes: '' },
      ...
      { id: uid(), weekId: 'w5', title: 'Advanced DP: Knapsack, LIS, longest common subsequence (8 problems)', status: 'todo', notes: '' },
    ]
  },
  {
    id: 'w6', number: 6,
    ...
    tasks: [
      ...
      { id: uid(), weekId: 'w6', title: 'Complete 2 DSA mock interviews (Pramp, Interviewing.io)', status: 'todo', notes: '' },
      { id: uid(), weekId: 'w6', title: 'Complete 1 system design mock interview', status: 'todo', notes: '' },
      { id: uid(), weekId: 'w6', title: 'Review and refine story bank based on mock feedback', status: 'todo', notes: '' },
      { id: uid(), weekId: 'w6', title: 'Practice behavioral answers out loud (record yourself)', status: 'todo', notes: '' },
      { id: uid(), weekId: 'w6', title: 'Strings & Advanced Patterns: Trie, Backtracking (8 problems)', status: 'todo', notes: '' },
    ]
  },
  {
    id: 'w7', number: 7,
    ...
    tasks: [
      ...
      { id: uid(), weekId: 'w7', title: 'Do a full system design round under timed conditions', status: 'todo', notes: '' },
    ]
  },
  {
    id: 'w8', number: 8,
    ...
    tasks: [
      { id: uid(), weekId: 'w8', title: 'Research market compensation data (levels.fyi, Glassdoor)', status: 'todo', notes: '' },
      { id: uid(), weekId: 'w8', title: 'Prepare negotiation script and counter-offer strategy', status: 'todo', notes: '' },
      { id: uid(), weekId: 'w8', title: 'Evaluate offer packages (base, equity, bonus, PTO, growth)', status: 'todo', notes: '' },
...
const sdTopics: SystemDesignTopic[] = [
  { id: uid(), title: 'URL Shortener (like bit.ly)', status: 'not-started', tradeoffs: '', notes: '', diagramRef: '', category: 'exercise' },
  { id: uid(), title: 'Notification Service (Email/SMS/Push)', status: 'not-started', tradeoffs: '', notes: '', diagramRef: '', category: 'exercise' },
  { id: uid(), title: 'Rate Limiter', status: 'not-started', tradeoffs: '', notes: '', diagramRef: '', category: 'exercise' },
  { id: uid(), title: 'Pastebin', status: 'not-started', tradeoffs: '', notes: '', diagramRef: '', category: 'exercise' },
  { id: uid(), title: 'File Upload Service (like S3)', status: 'not-started', tradeoffs: '', notes: '', diagramRef: '', category: 'exercise' },
  { id: uid(), title: 'Analytics Pipeline', status: 'not-started', tradeoffs: '', notes: '', diagramRef: '', category: 'exercise' },
  { id: uid(), title: 'Load Balancing & CDN', status: 'not-started', tradeoffs: '', notes: '', diagramRef: '', category: 'core' },
  { id: uid(), title: 'Caching Strategies (Redis, Memcached)', status: 'not-started', tradeoffs: '', notes: '', diagramRef: '', category: 'core' },
  { id: uid(), title: 'Database Sharding & Replication', status: 'not-started', tradeoffs: '', notes: '', diagramRef: '', category: 'core' },
  { id: uid(), title: 'Message Queues (Kafka, RabbitMQ)', status: 'not-started', tradeoffs: '', notes: '', diagramRef: '', category: 'core' },
  { id: uid(), title: 'CAP Theorem & Consistency Models', status: 'not-started', tradeoffs: '', notes: '', diagramRef: '', category: 'core' },
];

// ─── APPLICATIONS ──────────────────────────────────────────────────────────────
const applications: Application[] = [
  { id: uid(), company: 'Stripe', role: 'Senior Backend Engineer', location: 'Remote (India)', comp: '₹45-55 LPA', status: 'wishlist', notes: 'Strong infra team. Focus on payment systems SD.', referral: '', dates: [], createdAt: new Date().toISOString() },
  { id: uid(), company: 'Razorpay', role: 'Software Engineer II - Backend', location: 'Bangalore', comp: '₹35-45 LPA', status: 'applied', notes: 'Applied through company site. Backend platform team.', referral: '', dates: [], createdAt: new Date().toISOString() },
  { id: uid(), company: 'Zepto', role: 'Senior SDE - Backend', location: 'Mumbai', comp: '₹30-40 LPA', status: 'referred', notes: 'Referred by college friend Arjun who works on the data team.', referral: 'Arjun Kumar (Data Engineer)', dates: [], createdAt: new Date().toISOString() },
];

// ─── MOCK INTERVIEWS ───────────────────────────────────────────────────────────
const mockInterviews: MockInterview[] = [
  { id: uid(), type: 'DSA', date: new Date(Date.now() - 5 * 86400000).toISOString().slice(0, 10), score: 6, wrongPoints: ['Couldn\'t optimize brute force to O(n log n)', 'Forgot edge case: empty array'], improvements: 'Practice more heap problems. Review sorting-based optimizations.', interviewer: 'Pramp' },
];

// ─── NOTES ─────────────────────────────────────────────────────────────────────
const notes: Note[] = [
  { id: uid(), title: 'Two Pointer Pattern Notes', content: '## Two Pointer\n\nUse when:\n- Array is sorted or you need to find a pair\n- In-place array manipulation\n\n**Template:**\n```\nleft = 0, right = n-1\nwhile left < right:\n  if condition: return\n  elif too small: left++\n  else: right--\n```\n\nProblems: Two Sum II, Container With Most Water, 3Sum', category: 'dsa', tags: ['arrays', 'two-pointer'], createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() },
  { id: uid(), title: 'Stripe Interview Tips', content: '- Heavy focus on distributed systems\n- Expect 2 DSA rounds + 1 system design + 1 behavioral\n- System design usually around payments, reliability, or API design\n- Be ready to discuss failure handling and idempotency', category: 'company', tags: ['stripe', 'interview-prep'], createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() },
];
...
export const seedState: AppState = {
  weeks,
  dayLogs: [
    { id: uid(), date: yesterday, focusArea: 'DSA', plannedTime: 3, completedTime: 2.5, reflection: 'Good session on two-pointer problems. Need to revisit sliding window edge cases.' },
