...
function AppModal({ app, onClose, onSave }: { app: Partial<Application> & { id?: string }; onClose: () => void; onSave: (a: any) => void }) {
      ...
      <div className="modal modal-lg" onClick={e => e.stopPropagation()}>
        ...
        <div className="grid-2" style={{ gap: 12 }}>
          <div className="form-group"><label className="form-label">Company</label>
            <input className="form-input" value={form.company} onChange={e => set('company', e.target.value)} placeholder="Stripe, Google…" />
          </div>
          <div className="form-group"><label className="form-label">Role</label>
            <input className="form-input" value={form.role} onChange={e => set('role', e.target.value)} placeholder="Senior Backend Engineer…" />
          </div>
          <div className="form-group"><label className="form-label">Location</label>
            <input className="form-input" value={form.location} onChange={e => set('location', e.target.value)} placeholder="Bangalore / Remote…" />
          </div>
          <div className="form-group"><label className="form-label">Comp Target</label>
            <input className="form-input" value={form.comp} onChange={e => set('comp', e.target.value)} placeholder="₹40–50 LPA" />
          </div>
          ...
          <div className="form-group"><label className="form-label">Referral Info</label>
            <input className="form-input" value={form.referral} onChange={e => set('referral', e.target.value)} placeholder="Name, relationship…" />
          </div>
        </div>
        <div className="form-group"><label className="form-label">Notes</label>
          <textarea className="form-textarea" value={form.notes} onChange={e => set('notes', e.target.value)} placeholder="Key info, culture notes, team…" />
        </div>
        {/* Interview dates */}
        <div className="divider" />
        <div className="section-title" style={{ fontSize: '0.85rem' }}>Interview Dates</div>
        {(form.dates || []).map((d: InterviewDate) => (
          <div key={d.id} className="flex items-center gap-8 mb-8" style={{ fontSize: '0.82rem', color: 'var(--text-secondary)' }}>
        ...
        <div className="flex gap-8">
          <input type="date" className="form-input" style={{ flex: 1 }} value={dateForm.date} onChange={e => setDateForm(f => ({ ...f, date: e.target.value }))} />
          <select className="form-select" style={{ flex: 1 }} value={dateForm.type} onChange={e => setDateForm(f => ({ ...f, type: e.target.value }))}>
        ...
        <div className="flex gap-8 mt-16" style={{ justifyContent: 'flex-end' }}>
          ...
          <button className="btn btn-primary" onClick={() => { if (form.company.trim()) { onSave(form); onClose(); } }}>Save</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div>
      <div className="page-header">
        <div className="page-header-left"><h1>Job Applications</h1><p>Track your pipeline from wishlist to offer</p></div>
        <button className="btn btn-primary" onClick={() => setModal(emptyApp())}><Plus size={14} /> Add Application</button>
      </div>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 20 }}>
        {STATUSES.map(s => {
          ...
          if (!cnt) return null;
          return (
            <div key={s.key} className="card" style={{ padding: '8px 14px', display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', borderColor: filterStatus === s.key ? s.color : undefined }}
      ...
      <div className="flex gap-8 mb-16">
        ...
        {filterStatus && <button className="btn btn-ghost btn-sm" onClick={() => setFilterStatus('')}>Clear filter</button>}
      </div>
            ...
            >
              ...
              {col.map(app => (
                ...
                >
                  ...
                  {app.referral && <div style={{ fontSize: '0.68rem', color: 'var(--accent-light)', marginTop: 4 }}>👤 {app.referral}</div>}
                  {app.dates.length > 0 && (
                  ...
                  <div className="flex gap-4" style={{ marginTop: 8 }}>
                    <select
                      ...
                      onChange={e => dispatch({ type: 'UPDATE_APPLICATION_STATUS', id: app.id, status: e.target.value as AppStatus })}
                    >
                      {STATUSES.map(s2 => <option key={s2.key} value={s2.key}>{s2.label}</option>)}
                    </select>
                    <button className="btn btn-ghost btn-icon" onClick={e => { e.stopPropagation(); dispatch({ type: 'DELETE_APPLICATION', id: app.id }); }}>
                      <Trash2 size={11} style={{ color: 'var(--danger)' }} />
                    </button>
                  </div>
                </div>
              ))}
              {col.length === 0 && <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', textAlign: 'center', padding: 12 }}>—</div>}
            </div>
          );
        })}
      </div>
