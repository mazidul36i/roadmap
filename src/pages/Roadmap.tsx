...
import { ChevronDown, CheckCircle2, Circle, Clock, StickyNote, X } from 'lucide-react';
import { useApp, computeWeekProgress } from '../context/AppContext';
...
export default function Roadmap() {
  ...
  const toggle = (id: string) => setOpenWeeks(s => { const n = new Set(s); n.has(id) ? n.delete(id) : n.add(id); return n; });
  const toggleNote = (id: string) => setOpenNotes(s => { const n = new Set(s); n.has(id) ? n.delete(id) : n.add(id); return n; });
            ...
            <div className="accordion-header" onClick={() => toggle(week.id)}>
              <div style={{ flex: 1 }}>
                <div className="flex items-center gap-12 mb-8">
                  ...
                  <span style={{ fontWeight: 700, fontSize: '0.95rem', color: 'var(--text-primary)' }}>{week.title}</span>
                      ...
                      {noteOpen && (
                        <div style={{ padding: '8px 0 8px 34px' }}>
                          <textarea
                            ...
                            value={task.notes}
                            onChange={e => dispatch({ type: 'UPDATE_TASK_NOTES', weekId: week.id, taskId: task.id, notes: e.target.value })}
                          />
                        </div>
                      )}
                    </div>
