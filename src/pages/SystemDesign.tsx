...
const STATUS_LABELS: Record<SDStatus, string> = { 'not-started': 'Not Started', 'in-progress': 'In Progress', 'done': 'Done' };
const STATUS_BADGE: Record<SDStatus, string> = { 'not-started': 'badge-muted', 'in-progress': 'badge-warning', 'done': 'badge-success' };

const emptyTopic = (): Omit<SystemDesignTopic, 'id'> => ({
  title: '', status: 'not-started', tradeoffs: '', notes: '', diagramRef: '', category: 'exercise'
});

function TopicModal({ topic, onClose, onSave }: { topic: Partial<SystemDesignTopic> & { id?: string }; onClose: () => void; onSave: (t: any) => void }) {
      ...
      <div className="modal modal-lg" onClick={e => e.stopPropagation()}>
        ...
        <div className="grid-2" style={{ gap: 12 }}>
          <div className="form-group"><label className="form-label">Title</label>
            <input className="form-input" value={form.title} onChange={e => set('title', e.target.value)} placeholder="e.g. URL Shortener" />
          </div>
          <div className="grid-2" style={{ gap: 12 }}>
        ...
        <div className="form-group"><label className="form-label">Tradeoffs Learned</label>
          <textarea className="form-textarea" style={{ minHeight: 80 }} value={form.tradeoffs} onChange={e => set('tradeoffs', e.target.value)} placeholder="CAP tradeoffs, latency vs consistency…" />
        </div>
        <div className="form-group"><label className="form-label">Architecture Notes</label>
          <textarea className="form-textarea" style={{ minHeight: 120 }} value={form.notes} onChange={e => set('notes', e.target.value)} placeholder="Components, data flow, bottlenecks…" />
        </div>
        <div className="form-group"><label className="form-label">Diagram Reference (URL or text)</label>
          <input className="form-input" value={form.diagramRef} onChange={e => set('diagramRef', e.target.value)} placeholder="https://excalidraw.com/… or 'see Notion'" />
        </div>
        <div className="flex gap-8" style={{ justifyContent: 'flex-end' }}>
          <button className="btn btn-ghost" onClick={onClose}>Cancel</button>
          <button className="btn btn-primary" onClick={() => { if (form.title.trim()) { onSave(form); onClose(); } }}>Save</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div>
      <div className="page-header">
        <div className="page-header-left"><h1>System Design</h1><p>Track design exercises and core concept study</p></div>
      ...
      <div className="grid-auto">
        {topics.map(t => (
          <div key={t.id} className="card" style={{ display: 'flex', flexDirection: 'column', gap: 12, cursor: 'default' }}>
            <div className="flex justify-between items-start">
              ...
              <div className="flex gap-4">
                ...
                <button className="btn btn-ghost btn-icon" onClick={() => dispatch({ type: 'DELETE_SD_TOPIC', id: t.id })}>
